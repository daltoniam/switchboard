...
- **GitLab (`gitlab`)**: proxy to GitLab's official Streamable HTTP MCP (`https://<host>/api/v4/mcp`; `credentials.base_url` is the instance root, default `https://gitlab.com`). Auth via MCP OAuth (`mcp` scope, DCR) or personal access token as Bearer. Proxies the full upstream tool catalog as `gitlab_*` with enriched descriptions on MR/CI entry tools and `compact.yaml` on high-traffic reads. Phase 2 native REST for MCP-disabled instances is planned separately.
