...
- **GitLab (`gitlab`)**: native GitLab REST API v4 (`credentials.base_url` instance root, default `https://gitlab.com`; Switchboard calls `/api/v4`). Personal access token via `credentials.token` (`PRIVATE-TOKEN` header). Scoped tools for projects, merge requests (list/get/comment), issues, and CI/CD pipelines/jobs/logs with `compact.yaml` on read tools.
