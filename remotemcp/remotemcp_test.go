...
func TestConfigure_Success(t *testing.T) {
	i := New("test", "https://example.com")
	err := i.Configure(mcp.Credentials{"access_token": "tok123"})
...
func TestConfigure_TokenChange_ResetsSession(t *testing.T) {
	...
	err := r.Configure(mcp.Credentials{"access_token": "tok1"})
	assert.NoError(t, err)
	assert.Equal(t, "tok1", r.token)

	err = r.Configure(mcp.Credentials{"access_token": "tok2"})
	assert.NoError(t, err)
...
func (m *mockIntegration) Configure(mcp.Credentials) error { return nil }
