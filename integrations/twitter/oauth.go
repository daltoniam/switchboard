...
func StartTwitterOAuth(clientID, clientSecret, redirectURI string) (*OAuthStartResult, error) {
	if clientID == "" {
...
func HandleTwitterCallback(code, state string) error {
	...
	if oauthState.clientSecret != "" {
		req.SetBasicAuth(oauthState.clientID, oauthState.clientSecret)
	}
...
func PollTwitterOAuth() OAuthPollResult {
	oauthState := getActiveOAuth()
