...
func buildTools() []mcp.ToolDefinition {
	definitions := []mcp.ToolDefinition{
		...
		{Name: "paperless_get_document_thumbnail", Description: "Get a compact WebP image for visual inspection of a Paperless-ngx document. Start here for fast vision tasks such as reading a static image or evaluating the first page and rotation of a PDF; use get_document_preview when the original image resolution is necessary.", Parameters: map[string]string{"document_id": "Document ID from list_documents or search_documents"}, Required: []string{"document_id"}},
		{Name: "paperless_get_document_preview", Description: "Get a Paperless-ngx document preview as native image content when the stored or archived file is an image. Prefer get_document_thumbnail for PDFs because this endpoint returns a PDF rather than a model-ready image.", Parameters: map[string]string{"document_id": "Document ID from list_documents or search_documents"}, Required: []string{"document_id"}},
		{Name: "paperless_get_document_ocr_text", Description: "Get a paginated slice of OCR text for a Paperless-ngx document. Use with get_document_thumbnail when both visual layout or rotation and readable text are needed; continue with next_offset while has_more is true.", Parameters: map[string]string{"document_id": "Document ID", "offset": "Zero-based OCR character offset (default: 0)", "limit": "OCR characters to return (default: 10000, maximum: 20000)"}, Required: []string{"document_id"}},
...
func buildDispatch() map[mcp.ToolName]handlerFunc {
	d := map[mcp.ToolName]handlerFunc{
		"paperless_list_documents": listDocuments, "paperless_search_documents": searchDocuments, "paperless_create_document": createDocument, "paperless_get_document": getDocument, "paperless_get_document_thumbnail": getDocumentThumbnail, "paperless_get_document_preview": getDocumentPreview, "paperless_get_document_ocr_text": getDocumentOCRText, "paperless_update_document": updateDocument, "paperless_delete_document": deleteDocument, "paperless_download_document": downloadDocument,
